...
var userTemplateParam = script.addIntParameter("User Template selected","The page selected on your LaunchControl XL",0,0,8);
...
function scriptParameterChanged(param)
{
	...
	if(param.is(userTemplateParam))
	{
		local.sendSysex(0,32,41,2,17,119,param.get());
	}
}
...
function sysExEvent(data)
{
	...
	var pageSelectionMessage = [0,32,41,2,17,119,1];

	var checker = true;
	// Check if it's a page selected message
	for(var i = 0; i < 6; i++)
	{
		if(data[i] != pageSelectionMessage[i])
		{
			checker = false;
			break;
		}
